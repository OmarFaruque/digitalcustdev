...
if ( ! class_exists( 'Zoom_Video_Conferencing_Api' ) ) {
	...
	class Zoom_Video_Conferencing_Api {
		...
		public function createAMeeting( $data = array() ) {
			...
			$dt = new DateTime($data['start_date'], new DateTimezone($data['timezone']));
			$dt->setTimezone($tz);
		...
		public function updateMeetingInfo( $update_data = array() ) {
			// $dt = new DateTime($update_data['start_date'], new DateTimezone($update_data['timezone']));
			// $start_time = $dt->format('Y-m-d\TH:i:s');
		...
		public function newReport($user_id, $reportFrom, $reportTo){
			...
			return $this->sendRequest('report/users/' . $user_id . '/meetings?from=' . $reportFrom .'&page_size=300+&to=' . $reportTo .  '&type=past','GET');
		}
