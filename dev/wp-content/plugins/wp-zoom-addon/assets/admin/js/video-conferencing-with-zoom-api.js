...
(function ($) {
    ...
    $(document).on('click', '.zoom-help-notice button', function () {
        // Read the "data-notice" information to track which notice
        // is being dismissed and send it via AJAX
        var type = $('.zoom-help-notice').data('notice');
        // Make an AJAX call
        ...
        $.ajax(ajaxurl,
            {
                type: 'POST',
                data: {
                    action: 'video_conferencing_zoom_api_dismissed_notice_handler',
                    type: type,
                }
            });
    });
    ...
    var ZoomAPIJS = {
        ...
        eventListeners: function () {
            ...
            * End Meeting 
            */
           $("button[name='end_meeting']").on('click',this.endMeeting);  
        ...
        endMeeting: function() {
            ...
            var r = confirm("Confirm End this Meeting?");
            
            if (r == true) {
                var end_btn = $(this);
                var meeting_id = end_btn.val();
                var start_btn = `<a class="button start-meeting-btn reload-meeting-started-button" target="_blank" href="https://zoom.us/wc/${meeting_id}/start">Start Meeting</a>`;
                end_btn.replaceWith(start_btn);
                var img_started = $('img.started');
                var start_link = $('img.ended').attr('src');
                img_started.attr('src',start_link);
                var data = {meeting_id: meeting_id,  action: 'zvc_end_meeting', security: zvc_ajax.zvc_security};
                $dom.cover.show();
                $.post(zvc_ajax.ajaxurl, data).done(function (result) {
                    $dom.cover.fadeOut('slow');
                    ...
                    if (result.error == 1) {
                        ...
                        $dom.show_on_meeting_delete_error.show().html('<p>' + result.msg + '</p>');
                        $(this).children('<tr>').load(location.href + $(this).children('<tr>'));
