...
// $webnar = true;
// if(isset($_REQUEST['s']) && isset($_REQUEST['ref'])) $webnar = false;
?>
...
<?php } ?>
	...
	<div id="thim-course-archive" class="om5  <?php echo ( $layout == 'list-layout' ) ? 'thim-course-list' : 'thim-course-grid'; ?>" data-cookie="grid-layout">
		<?php if ( LP()->wp_query->have_posts() ) : ?>

			<?php while ( LP()->wp_query->have_posts() ) : LP()->wp_query->the_post(); ?>
				<?php //if(!$webnar && get_post_meta($post->ID, '_course_type', true) != 'webinar' ): ?>
