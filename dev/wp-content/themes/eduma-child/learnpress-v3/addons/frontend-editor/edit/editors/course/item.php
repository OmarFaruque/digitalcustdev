...
<script type="text/x-template" id="tmpl-e-course-item">
    <li :class="itemClasses()" tabindex="0" @focus="onFocus" @click="onFocus($event)" @blur="onBlur" :data-id="item.id || ''"
        ...
        <div class="item-title-input">
            <input type="text" v-model="item.title" data-context="item-title" class="item-title item-lesson-title-input"
                   @mousedown="_select"
                   :placeholder="getInputPlaceholderText('<?php _e( 'Item name', 'learnpress-frontend-editor' ); ?>')">
        </div>
        <div class="item-title-add-item">
            <button type="button" class="e-button item-lesson-add-btn" @click="_addNewLesson" style="display: none;"><i class="fa fa-2x fa-plus-circle"></i></button>
        </div>
        <div class="e-sortable-actions e-item-actions om">
