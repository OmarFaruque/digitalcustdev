...
function add_body_tag_manager() { ?>
    ...
    <noscript>
        <iframe src="https://www.googletagmanager.com/ns.html?id=GTM-WJBL23J"
...
function thim_child_enqueue_styles() {
	if ( is_multisite() ) {
		...
		wp_enqueue_style( 'thim-custom-style', get_stylesheet_directory_uri() . '/assets/css/styles.min.css', array(), THIM_THEME_VERSION );

		if ( is_page( 'profile' ) ) {
			wp_enqueue_script( 'thim-popper-js', get_stylesheet_directory_uri() . '/assets/js/popper.min.js', array( 'jquery' ), THIM_THEME_VERSION, true );
		}

		wp_enqueue_script( 'thim-custom-js', get_stylesheet_directory_uri() . '/assets/js/child-theme-custom.js', array( 'jquery' ), THIM_THEME_VERSION, true );
		$localize_array = array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) );
		wp_localize_script( 'thim-custom-js', 'digitalcustdev', $localize_array );
	}
}

//Requiring extra files
require_once get_stylesheet_directory() . '/inc/frontend-editor/functions.php';
require_once get_stylesheet_directory() . '/inc/frontend-editor/course.php';
require_once get_stylesheet_directory() . '/inc/class-ajax.php';
...
function slt_PHPErrorsWidget() {
	...
	if ( file_exists( $logfile ) ) {
		...
		if ( $errors ) {
			...
			echo '.';
			if ( $userCanClearLog ) {
				echo ' [ <b><a href="' . get_bloginfo( "url" ) . '/wp-admin/?slt-php-errors=clear" onclick="return confirm(\'Вы уверенны?\');">ОЧИСТИТЬ ЛОГ ФАЙЛ</a></b> ]';
			}
			echo '</p>';
...
add_action( 'after_setup_theme', 'dcd_image_sizes' );
function dcd_image_sizes() {
	add_image_size( 'featured-cover', 870, 500 );
}


if ( ! function_exists( 'thim_widgets_2_inits' ) ) {
	function thim_widgets_2_inits() 
	{
		register_sidebar( array(
			'name'          => esc_html__( 'Menu Right 2', 'eduma' ),
			'id'            => 'menu_right_2',
			'description'   => esc_html__( 'Menu Right 2', 'eduma' ),
...
add_action( 'widgets_init', 'thim_widgets_2_inits' );

// register new webinar taxonomy
// add_action( 'init', 'webinar_add_taxonomy' );
//     function webinar_add_taxonomy() {
//         $labels = array(
//         'name' => _x( 'Webinar_categories', 'taxonomy general name' ),
...
//         register_taxonomy('Webinar_Category', 'lp_course', array(
...
//             )
//         ));
//         register_taxonomy('webinar_tag','lp_course',
//             array(
//             'hierarchical' => false,
