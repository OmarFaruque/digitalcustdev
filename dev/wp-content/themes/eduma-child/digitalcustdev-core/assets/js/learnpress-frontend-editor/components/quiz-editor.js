...
//ff
    Vue.component('e-question-loop', {
        ...
        methods: $.extend(FE_Base.Store_Methods, {
            ...
            _addNewQuestionChild: function (e) {
                var question_title = this.$refs.questionTitle;
                if (!(question_title.value + '').length) {
                    e.preventDefault();
                    return;
                }

                this.$emit('press-question-add-btn', [e, this]);
            },
            _switch: function (e) {
                var $vm = this,
                    $li = $(e.target),
            ...
            _duplicate_question: function (e, id) {
                this.$emit('clone-question', {
                    event: e,
                    $vm: this,
                    question_id: id
                });
    ...
    Vue.component('e-quiz-editor', {
        ...
        created: function () {
            ...
            //Added by Deepen
            this.defaultQuestion.type = 'single_choice';
        ...
        methods: $.extend({}, FE_Base.Store_Methods, {
            ...
            _onQuestionAddBtnClick: function (args) {
                var $question = $(args[0].target).closest('.e-question-loop'),
                ...
                $('.item-question-add-btn').addClass('disabled');
            },
            _onKeydownQuestionTitle: function (args) {
                ...
                var title_length = args[1].$refs.questionTitle.value.length + 1;
                ...
                if (title_length > 3) {
                    $(args[0].target).closest('.e-question-loop').find('.item-question-add-btn').removeClass('disabled');
                } else {
                    $(args[0].target).closest('.e-question-loop').find('.item-question-add-btn').addClass('disabled');
                    return false;
                }
            ...
            _onCloneQuestion: function (args) {
                var me = this;

                var questionData = args.$vm.question;
                var $question = $(args.$vm.$el),
                    $questions = $question.parent().children(),
                    position = $questions.index($question);
                this.$root.$request('', 'clone-question', {
                    quiz_ID: this.itemData.id,
                    question: questionData
                }).then(function (r) {
                    if (r.result === 'success') {
                        if (r.id && questionData.id !== r.id) {
                            questionData.id = r.id;
                            Vue.set(questionData, 'answers', r.answers || []);
                        }

                        me.addQuestion(position + 1, questionData);
                    }
                });
            }
