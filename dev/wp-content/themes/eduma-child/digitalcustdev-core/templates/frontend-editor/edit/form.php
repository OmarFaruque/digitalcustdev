...
$duration_term = get_post_meta( $post->ID, '_lp_duration', true );
$youtube_link  = get_post_meta( $post->ID, 'thim_course_media_intro', true );
if ( ! empty( $duration_term ) ) {
	$duration_term = explode( ' ', $duration_term );
}
?>
<div class="row">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <select class="form-control select2-select  " name="webinar_duration_term">
                    ...
                    <option <?php selected( 'week', $duration_term[1] ); ?> value="week">Week(s)</option>
                </select>
            </div>
        </div>
    </div>
